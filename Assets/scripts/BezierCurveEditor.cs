...
public class BezierCurveEditor : Editor
{
    ...
    override public void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Add Point Idiot"))
        {
            (target as BezierCurve).AddPoint();
            EditorUtility.SetDirty(target);
        }
        // GUI.Button(new Rect(0, 0, 100, 30), "Add Point Idiot");
    }
    ...
    void OnSceneGUI()
    {
        ...
        if (selectedIndex >= 0)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                curve.worldPoints[selectedIndex] = curve.transform.InverseTransformPoint(newPos);
                curve.CachedSplineData();
            }
        }
